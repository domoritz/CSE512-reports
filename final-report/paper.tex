...
\newcommand*{\overview}{Fragment Overview\xspace}
...
The \overview shows small multiples of the utilization chart as described in the previous section.
...
We explore a simple join query we performed on one million tuples extracted from Twitter data. This query joins Twitter followers and followees by user ID. The query written in SQL is expressed below.
...
The resulting \graph visualization is displayed in ~\ref{fig:TODO}. The user will navigate the \graph visualization in the browser panel in the next case studies to identify several performance symptoms.
...
\subsection{Case 1: Identifying Performance Bottlenecks using the \overall View}

%dom

\subsection{Case 1: Identifying Data Skew using the \network View}
