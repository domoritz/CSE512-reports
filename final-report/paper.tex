...
We propose \system, a visualization tool that helps database developers
...
visualization techniques to present: (1) data flow between query operators and
...
communication between distributed workers. \system is designed to help the Myria developer answer the following questions regarding
their query execution. 
\system, uses profiling logs from Myria as the prototype DDMS. Myria is a distributed
database platform that provides \emph{big data management and analytics as a service} primarily for scientific applications.
Myria takes queries in high-level query languages, such as SQL, translating them into physical query plans and finally executing
them in parallel. Currently, Myria does not offer a visualization tool designed to show the physical query plan, job execution and network
communication between distributed workers. \system allows the user to visualize the following aspects
of query execution.
...
%The physical
%query plan is composed of a directed graph of query \emph{fragments}. A query
...
%operators inside the fragment, but on different data. Each graph link represents
%communication between workers, possibly over the network.  \system displays
%this \graph in an interactive fashion with clickable elements for fragment nodes
%and communication links.  The \graph also represents a high-level data flow in
%the DDBMS.

\item \emph{How does the cluster visualization vary for each fragment and operator?} 
%A view for cluster utilization and operator performance for each query fragment. 
...
executing an operator. \system provides a \fragment view to aid this understanding. 
%Upon clicking on a collapsed fragment node in the
%query plan DAG, the node expands to display the operators comprising the fragment and renders the fragment execution view.
The \fragment view visualizes the utilization of the cluster by showing whether each worker is busy working
...
%\network view. This shows the aggregate number of tuples exchanged between workers as well as a time series
...
techniques that we found fit to allow developers to get insight
...
each other. We designed a view that allows the developer to analyze
...
\begin{itemize}
    ...
    \item \textbf{Edge selection:} upon selecting one or more fragment-to-fragment edges, a \network visualization gets displayed, providing information on inter-\network from one query fragment to the next.
...
\begin{figure}[ht]
  ...
  \caption{The utilization chart. The user can brush on either the small graph on top or the bottom graph. Brushing on the second chart zooms the chart.}
...
for the \network chart. But feedback during the progress presentation session convinced us that
