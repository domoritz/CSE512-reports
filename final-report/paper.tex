...
is called \system, which uses profiling logs from \emph{Myria} \cite{myria} as the prototype DDMS. Myria is a distributed 
...
\item A view for cluster utilization and operator performance for each \emph{Myria} segment. Each segment is composed of
multiple sequential database operators running on the same worker. Upon clicking on a collapsed segment node in the
query plan DAG, the node expands to display the operators comprising the segment and renders the fragment execution view.
...
with the selected link in the query plan DAG. 

\begin{figure}[ht]
  \includegraphics[width=\columnwidth]{images/networkVis1}
  \caption{Worker communication matrix chart.}
  \label{fig:matrix}
\end{figure}
